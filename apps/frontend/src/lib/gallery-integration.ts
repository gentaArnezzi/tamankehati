  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/galleries/`, {
      ...
      body: JSON.stringify({
        ...
        entity_id: typeof metadata.entityId === 'string' ? parseInt(metadata.entityId) : metadata.entityId,
    ...
    if (!response.ok) {
      ...
      console.warn('Failed to create gallery record:', response.status, errorText);
