...
export async function getTamanList(params?: {
  search?: string;
  status?: string;
  region?: string;
  limit?: number;
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/public/parks?${queryParams.toString()}`,
    );
    if (!response.ok) {
      throw new Error(`HTTP ${response.status}: ${response.statusText}`);
    }
...
export async function fetchGalleryPage(params: SearchParams = {}) {
  try {
    const response = await fetch(
      `/api/public/galeri/?${new URLSearchParams(params as Record<string, string>).toString()}`,
    );
