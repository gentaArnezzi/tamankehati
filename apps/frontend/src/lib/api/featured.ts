...
export async function fetchFeaturedFlora(
  limit: number = 1,
): Promise<FeaturedItem[]> {
  try {
    // Try to fetch flora with IUCN status first (CR, EN, VU), then fallback to any flora
    let response = await fetch(
      `${API_BASE_URL}/api/public/flora?limit=${limit}&sort=created_at_desc`,
    );
    if (!response.ok) {
...
export async function fetchFeaturedFauna(
  limit: number = 1,
): Promise<FeaturedItem[]> {
  try {
    // Fetch fauna sorted by most recent
    const response = await fetch(
      `${API_BASE_URL}/api/public/fauna?limit=${limit}&sort=created_at_desc`,
    );
    if (!response.ok) {
...
export async function fetchFeaturedParks(
  limit: number = 1,
): Promise<FeaturedItem[]> {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/public/parks?limit=${limit}`,
    );
    if (!response.ok) {
