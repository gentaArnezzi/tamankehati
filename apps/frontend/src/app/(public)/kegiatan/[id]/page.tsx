...
export async function generateMetadata({ params }: ActivityDetailPageProps): Promise<Metadata> {
  try {
    ...
    return {
      ...
      description: activity.description?.slice(0, 150) ?? 'Detail kegiatan dari portal Taman Kehati.',
      openGraph: {
        title: activity.title,
        description: activity.description?.slice(0, 150),
        images: activity.images && activity.images.length > 0 ? [{ url: activity.images[0] }] : undefined,
      },
    };
  } catch {
    return {
