...
export default function TanyaKehatiPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Halo! Saya Tanya Kehati, asisten AI untuk keanekaragaman hayati Indonesia. Saya dapat membantu Anda menemukan informasi tentang flora, fauna, taman konservasi, dan kegiatan konservasi. Silakan ajukan pertanyaan spesifik tentang keanekaragaman hayati Indonesia!",
    ...
    try {
      ...
      for (const keyword of floraKeywords) {
        if (
          ...
          const floraResults = await publicApi.getFlora({
            search: keyword,
            limit: 3,
          });
      ...
      for (const keyword of faunaKeywords) {
        if (
          ...
          const faunaResults = await publicApi.getFauna({
            search: keyword,
            limit: 3,
          });
      ...
      if (
        lowerQuestion.includes("taman") ||
        lowerQuestion.includes("konservasi") ||
        lowerResponse.includes("taman") ||
        lowerResponse.includes("konservasi")
      ) {
        const tamanResults = await publicApi.getTaman({
          search: "konservasi",
          limit: 3,
        });
  ...
  const getRelevantLinks = (question: string, response: string) => {
    ...
    if (
      lowerQuestion.includes("flora") ||
      lowerQuestion.includes("tumbuhan") ||
      lowerQuestion.includes("tanaman") ||
      lowerResponse.includes("flora") ||
      lowerResponse.includes("tumbuhan")
    ) {
      links.push({
    ...
    if (
      lowerQuestion.includes("fauna") ||
      lowerQuestion.includes("hewan") ||
      lowerQuestion.includes("satwa") ||
      lowerResponse.includes("fauna") ||
      lowerResponse.includes("hewan")
    ) {
      links.push({
    ...
    if (
      lowerQuestion.includes("taman") ||
      lowerQuestion.includes("konservasi") ||
      lowerQuestion.includes("taman nasional") ||
      lowerResponse.includes("taman") ||
      lowerResponse.includes("konservasi")
    ) {
      links.push({
      ...
      if (
        lowerQuestion.includes("terbesar") ||
        lowerQuestion.includes("besar") ||
        lowerQuestion.includes("luas")
      ) {
        links.push({
    ...
    if (
      lowerQuestion.includes("artikel") ||
      lowerQuestion.includes("berita") ||
      lowerQuestion.includes("kegiatan") ||
      lowerResponse.includes("artikel") ||
      lowerResponse.includes("berita")
    ) {
      links.push({
    ...
    if (
      lowerQuestion.includes("cari") ||
      lowerQuestion.includes("pencarian") ||
      lowerQuestion.includes("temukan")
    ) {
      links.push({
    ...
    if (
      lowerQuestion.includes("tentang") ||
      lowerQuestion.includes("apa itu") ||
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const isGenericResponse =
        aiResponse.includes("Maaf, saya tidak memiliki data spesifik") ||
        aiResponse.includes("tidak memiliki data") ||
        aiResponse.includes("database Taman Kehati saat ini");

      if (isGenericResponse) {
        console.warn(
          "AI returned generic fallback response - knowledge base may be empty",
        );
      ...
      const specificItems = await searchForSpecificItems(
        userMessage.content,
        aiResponse,
      );
      ...
      const isEmptyResponse =
        error instanceof Error &&
        error.message.includes("Empty response from AI");
      ...
      const errorMessage: Message = {
        ...
        role: "assistant",
        content:
          isNetworkError || isServiceUnavailable || isEmptyResponse
                ...
                >
                  {message.role === "assistant" &&
                    (message.content.includes(
                      "Maaf, saya tidak memiliki data spesifik",
                    ) ||
                      message.content.includes("tidak memiliki data") ||
                      message.content.includes(
                        "database Taman Kehati saat ini",
                      )) && (
        ...
        <div className="border-t border-slate-100 px-6 py-4 bg-white">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-xs text-slate-400 text-center mt-3">
              Tanya Kehati menggunakan AI untuk memberikan informasi tentang
