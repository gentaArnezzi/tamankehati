  ...
  const loadNewsDetail = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://103.125.91.16"}/api/v1/news/public/${params.id}`,
      );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            {news.is_pinned && <Pin className="h-4 w-4 text-blue-600" />}
            {news.is_featured && <Star className="h-4 w-4 text-yellow-600" />}
