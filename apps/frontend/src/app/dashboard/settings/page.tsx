...
export default function SettingsPage() {
  ...
  const handleUpdateNotifications = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "https://tamankehati-backend-pxnu.onrender.com"}/api/v1/users/me/notifications`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleUploadPhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "https://tamankehati-backend-pxnu.onrender.com"}/api/v1/users/me/upload-photo`,
        {
          method: "POST",
          headers: {
            Authorization: "Bearer " + token,
