...
import { Settings, User, Mail, Lock, Save, Bell, Shield, Eye, EyeOff, CheckCircle2, AlertCircle, Camera } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://tamankehati-backend-pxnu.onrender.com'}/api/v1/users/me/profile`, {
        method: 'PATCH',
        headers: {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://tamankehati-backend-pxnu.onrender.com'}/api/v1/users/me/change-password`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + token,
  ...
  const handleUpdateNotifications = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://tamankehati-backend-pxnu.onrender.com'}/api/v1/users/me/notifications`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + token,
  ...
  const handleUploadPhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://tamankehati-backend-pxnu.onrender.com'}/api/v1/users/me/upload-photo`, {
        method: 'POST',
        headers: {
          'Authorization': 'Bearer ' + token,
        },
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-muted-foreground text-lg">Kelola informasi akun dan preferensi Anda</p>
      </div>
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-brand-50 text-brand-700 border border-brand-200">
                  ...
                  {user?.role === 'regional_admin' && '🛡️ Regional Administrator'}
                </span>
            ...
            <CardContent>
              <dl className="space-y-4">
                <div className="flex items-center justify-between py-3 border-b">
                  ...
                  <dd className="text-sm font-mono font-semibold">{user?.id}</dd>
                </div>
                <div className="flex items-center justify-between py-3 border-b">
                  <dt className="text-sm font-medium text-muted-foreground">Role</dt>
                  <dd className="text-sm font-semibold">
                    {user?.role === 'super_admin' && 'Super Administrator'}
                    {user?.role === 'regional_admin' && 'Regional Administrator'}
                  </dd>
                </div>
        ...
        <TabsContent value="security" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Pastikan akun Anda menggunakan password yang kuat untuk keamanan maksimal
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleChangePassword} className="space-y-6">
                    ...
                    <div className="space-y-2 mt-3">
                      <div className="flex gap-1">
                        {[...Array(4)].map((_, i) => {
                          ...
                          const colorClass = isActive ? getPasswordStrengthColor() : 'bg-gray-200';
                          return (
                            <div
                              key={i}
                      ...
                      <p className="text-xs text-muted-foreground">
                        Kekuatan password: <span className="font-semibold">{getPasswordStrengthText()}</span>
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm text-blue-900 font-medium mb-2">Tips Password Kuat:</p>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between py-4 border-b">
                  ...
                  <button
                    ...
                    className={'relative inline-flex h-6 w-11 items-center rounded-full transition-colors ' + (notifications.emailNotifications ? 'bg-brand-600' : 'bg-gray-200')}
                  >
                    <span
                ...
                <div className="flex items-center justify-between py-4 border-b">
                  ...
                  <button
                    ...
                    className={'relative inline-flex h-6 w-11 items-center rounded-full transition-colors ' + (notifications.pushNotifications ? 'bg-brand-600' : 'bg-gray-200')}
                  >
                    <span
                ...
                <div className="flex items-center justify-between py-4 border-b">
                  <div className="space-y-1">
                    <Label className="text-sm font-medium">Pengumuman Baru</Label>
                    <p className="text-sm text-muted-foreground">Notifikasi untuk pengumuman baru</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setNotifications(prev => ({ ...prev, announcementAlerts: !prev.announcementAlerts }))}
                    className={'relative inline-flex h-6 w-11 items-center rounded-full transition-colors ' + (notifications.announcementAlerts ? 'bg-brand-600' : 'bg-gray-200')}
                  >
                    <span
                      className={'inline-block h-4 w-4 transform rounded-full bg-white transition-transform ' + (notifications.announcementAlerts ? 'translate-x-6' : 'translate-x-1')}
                    />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between py-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">Notifikasi untuk status persetujuan</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setNotifications(prev => ({ ...prev, approvalAlerts: !prev.approvalAlerts }))}
                    className={'relative inline-flex h-6 w-11 items-center rounded-full transition-colors ' + (notifications.approvalAlerts ? 'bg-brand-600' : 'bg-gray-200')}
                  >
                    <span
                      className={'inline-block h-4 w-4 transform rounded-full bg-white transition-transform ' + (notifications.approvalAlerts ? 'translate-x-6' : 'translate-x-1')}
                    />
                  </button>
                </div>
              </div>
              ...
              <div className="flex justify-end">
                <Button onClick={handleUpdateNotifications} disabled={loading} className="gap-2">
                  <Save className="h-4 w-4" />
