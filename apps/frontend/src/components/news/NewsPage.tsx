...
export function NewsPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/news/?${params}`,
        {
          headers: {
  ...
  const handleSubmit = async (formData: Partial<News>) => {
    try {
      const url = formMode === 'create' 
        ...
        : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/news/${formData.id}`;
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/news/${id}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
  ...
  const handlePublish = async (id: number) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/news/${id}/publish`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
  ...
  const handleArchive = async (id: number) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/news/${id}/archive`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
