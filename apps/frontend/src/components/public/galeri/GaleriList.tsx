...
export function GaleriList({ className = "" }: GaleriListProps) {
    ...
    return (
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {Array.from({ length: 8 }).map((_, index) => (
          <div
            key={index}
            className="overflow-hidden rounded-xl border bg-card text-card-foreground shadow-sm animate-pulse aspect-square"
          >
  ...
  return (
    <div
      className={`grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 ${className}`}
    >
