...
export function ActivityDetailView({ activity }: ActivityDetailViewProps) {
  const heroImage = activity.images && activity.images.length > 0 
    ? (activity.images[0].startsWith('http') ? activity.images[0] : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}${activity.images[0]}`)
            ...
            {activity.images.slice(1).map((image, index) => (
              ...
              >
                <div className="aspect-square relative overflow-hidden bg-slate-100">
                  <Image
                    src={image.startsWith('http') ? image : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}${image}`}
