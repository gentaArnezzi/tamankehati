  ...
  useEffect(() => {
    const fetchGalleryImages = async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://tamankehati-backend-pxnu.onrender.com'}/api/public/fauna/${fauna.id}/gallery`);
        if (response.ok) {
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <Breadcrumb>
                  <BreadcrumbList className="text-white/80">
                    ...
                    <BreadcrumbItem>
                      <BreadcrumbLink asChild>
                        <Link href="/fauna" className="hover:text-white transition-colors">Fauna</Link>
                      </BreadcrumbLink>
                    </BreadcrumbItem>
                    <BreadcrumbSeparator className="text-white/50" />
                    <BreadcrumbItem>
