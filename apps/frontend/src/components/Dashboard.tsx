...
export function Dashboard() {
  ...
  const loadRegionalAdminData = async () => {
    try {
      ...
      const [tamanData, floraData, faunaData, activitiesData] = await Promise.all([
        // Get parks created by this regional admin
        fetch(`${apiUrl}/api/v1/parks?submitted_by=me`, { headers }).then(res => res.json()).catch(() => ({ items: [] })),
        // Get flora submitted by this regional admin
        fetch(`${apiUrl}/api/v1/flora?submitted_by=me`, { headers }).then(res => res.json()).catch(() => ({ items: [] })),
        // Get fauna submitted by this regional admin
        fetch(`${apiUrl}/api/v1/fauna?submitted_by=me`, { headers }).then(res => res.json()).catch(() => ({ items: [] })),
        // Get activities submitted by this regional admin
        fetch(`${apiUrl}/api/v1/activities?submitted_by=me`, { headers }).then(res => res.json()).catch(() => ({ items: [] }))
      ]);
  ...
  const pendingApprovals = stats?.pending_approvals ?? stats?.pending_approval ?? 0;
              ...
              <>
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <MapPin className="h-4 w-4 text-muted-foreground" style={{ color: '#356447' }} />
                  </CardHeader>
                  <CardContent>
                    <div className="text-2xl">{formatNumber(totalObservasi)}</div>
                ...
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <TreePine className="h-4 w-4 text-muted-foreground" style={{ color: '#356447' }} />
                  </CardHeader>
                  <CardContent>
                ...
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <Bird className="h-4 w-4 text-muted-foreground" style={{ color: '#356447' }} />
                  </CardHeader>
                  <CardContent>
        ...
        <CardContent className="pt-6">
          {loading ? (
            ...
            <div className="space-y-3">
              {activity.map((item) => (
                <div key={item.id} className="flex items-start gap-4 p-4 rounded-xl hover:bg-gray-50 transition-colors group cursor-pointer">
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-gradient-to-br from-brand-100 to-brand-200 dark:from-brand-900 dark:to-brand-800 group-hover:scale-110 transition-transform">
                    {item.type === 'observasi' && <MapPin className="h-6 w-6 text-brand-600" />}
                    {item.type === 'flora' && <TreePine className="h-6 w-6 text-brand-600" />}
                    {item.type === 'fauna' && <Bird className="h-6 w-6 text-brand-600" />}
                    {item.type === 'taman' && <TreePine className="h-6 w-6 text-brand-600" />}
                  </div>
          ...
          <CardContent className="pt-6">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <button className="group p-6 rounded-2xl bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-200 hover:border-green-400 dark:hover:border-green-700 hover:shadow-lg transition-all duration-300 text-left">
                ...
                <p className="text-xs text-gray-900 mt-1 font-medium">Dokumentasi spesies baru</p>
              </button>
              <button className="group p-6 rounded-2xl bg-gradient-to-br from-blue-50 to-sky-50 dark:from-blue-950 dark:to-sky-950 border-2 border-blue-200 hover:border-blue-400 dark:hover:border-blue-700 hover:shadow-lg transition-all duration-300 text-left">
                ...
                <p className="text-xs text-gray-900 mt-1 font-medium">Catat satwa liar</p>
              </button>
              <button className="group p-6 rounded-2xl bg-gradient-to-br from-violet-50 to-purple-50 border-2 border-violet-200 hover:border-violet-400 dark:hover:border-violet-700 hover:shadow-lg transition-all duration-300 text-left">
                ...
                <p className="text-sm font-bold text-gray-900">Catat Observasi</p>
                <p className="text-xs text-gray-900 mt-1 font-medium">Laporan lapangan</p>
              </button>
              {user?.role === 'super_admin' && (
                <button className="group p-6 rounded-2xl bg-gradient-to-br from-orange-50 to-red-50 border-2 border-orange-200 hover:border-orange-400 dark:hover:border-orange-700 hover:shadow-lg transition-all duration-300 text-left">
                  ...
                  <p className="text-sm font-bold text-gray-900">Tinjau Persetujuan</p>
                  <p className="text-xs text-gray-900 mt-1 font-medium">Review data pending</p>
                </button>
              )}
            </div>
          </CardContent>
