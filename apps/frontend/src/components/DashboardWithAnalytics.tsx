...
export function DashboardWithAnalytics() {
  ...
  if (!data || !data.analytics || !data.analytics.biodiversity) {
    return (
      <Alert>
        <AlertCircle className="h-4 w-4" />
        <AlertDescription>
          Data dashboard tidak tersedia. Silakan refresh halaman atau hubungi
          administrator.
        </AlertDescription>
      </Alert>
    );
  ...
  const statusDistribution = [
    ...
    },
    {
      ...
      percentage:
        100 - (data.analytics.biodiversity.summary?.approval_rate ?? 0),
    },
  ];
  ...
  const conservationStatusData = [
    {
      status: "Endemik",
      count: data.analytics.biodiversity.summary?.total_endemic ?? 0,
      color: "#8B5CF6",
    },
    {
      status: "Non-Endemik",
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      >
        ...
        <TabsContent value="overview" className="space-y-4 sm:space-y-6">
          ...
          {data.analytics.biodiversity.monthly_discoveries &&
            data.analytics.biodiversity.monthly_discoveries.length > 0 && (
              <Card>
                <CardHeader>
          ...
          {data.analytics.biodiversity.regional_distribution &&
            data.analytics.biodiversity.regional_distribution.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <MapPin className="h-5 w-5" />
                    Distribusi Spesies per{" "}
                    {data.user_role === "super_admin" ? "Wilayah" : "Taman"}
                  </CardTitle>
                  <CardDescription>
        ...
        <TabsContent value="conservation" className="space-y-4 sm:space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-4xl font-bold text-purple-600">
                  {Math.round(
                    data.analytics.conservation.parks.avg_area_ha,
                  ).toLocaleString()}
                </div>
          ...
          {data.analytics.biodiversity.park_distribution &&
            data.analytics.biodiversity.park_distribution.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <TreePine className="h-5 w-5" />
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-sm">
                    {new Date(data.date_range.end).toLocaleDateString("id-ID")}
                  </span>
                </div>
                <div className="flex justify-between">
