...
export default function AIFloraFaunaGenerator() {
  ...
  const handleFloraDataChange = (field: keyof FloraData, value: string | boolean) => {
    setFloraData(prev => ({ ...prev, [field]: value }));
  };

  const handleFaunaDataChange = (field: keyof FaunaData, value: string | boolean) => {
  ...
  const generateDescription = async () => {
    ...
    try {
      const endpoint = type === 'flora' 
        ? `${process.env.NEXT_PUBLIC_API_URL || 'https://tamankehati-backend-pxnu.onrender.com'}/api/v1/ai/public/generate-flora-description'
        : `${process.env.NEXT_PUBLIC_API_URL || 'https://tamankehati-backend-pxnu.onrender.com'}/api/v1/ai/public/generate-fauna-description';
  ...
  const testOllamaConnection = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://tamankehati-backend-pxnu.onrender.com'}/api/v1/ai/public/test-ollama');
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Otomatisasi pembuatan deskripsi flora dan fauna menggunakan AI lokal (Ollama)
          </CardDescription>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                value={type === 'flora' ? floraData.local_name || '' : faunaData.local_name || ''}
                onChange={(e) => {
                  if (type === 'flora') {
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                value={type === 'flora' ? floraData.scientific_name || '' : faunaData.scientific_name || ''}
                onChange={(e) => {
                  if (type === 'flora') {
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                value={type === 'flora' ? floraData.family || '' : faunaData.family || ''}
                onChange={(e) => {
                  if (type === 'flora') {
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                value={type === 'flora' ? floraData.genus || '' : faunaData.genus || ''}
                onChange={(e) => {
                  if (type === 'flora') {
            ...
            <div className="space-y-2">
              ...
              <Select
                value={type === 'flora' ? floraData.iucn_status || '' : faunaData.iucn_status || ''}
                onValueChange={(value) => {
                  if (type === 'flora') {
            ...
            <div className="flex items-center space-x-2">
              <Switch
                id="is_endemic"
                checked={type === 'flora' ? floraData.is_endemic || false : faunaData.is_endemic || false}
