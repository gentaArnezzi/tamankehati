...
describe('ComprehensiveAIGenerator', () => {
  ...
  it('handles form input changes', () => {
    ...
    fireEvent.click(screen.getByText('Artikel'));
    const topicInput = screen.getByPlaceholderText('Contoh: Konservasi Harimau Sumatera');
  ...
  it('shows loading state during generation', async () => {
    (fetch as jest.Mock).mockImplementation(() => 
      new Promise(resolve => setTimeout(() => resolve({
        ok: true,
        json: () => Promise.resolve({ success: true, title: 'Test', content: 'Test content' })
      }), 100))
    );
    ...
    // Fill form
    fireEvent.change(screen.getByPlaceholderText('Contoh: Konservasi Harimau Sumatera'), {
    ...
    // Check loading state
    expect(screen.getByRole('button', { name: /Generate Artikel/ })).toBeDisabled();
  ...
  it('handles API errors gracefully', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Terjadi kesalahan saat generate artikel')).toBeInTheDocument();
    });
  });
  ...
  it('displays generated content correctly', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByDisplayValue('Test article summary')).toBeInTheDocument();
      expect(screen.getByDisplayValue('Test article content')).toBeInTheDocument();
    });
  });
});
