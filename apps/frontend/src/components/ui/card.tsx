...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-white text-gray-900 flex flex-col gap-6 rounded-xl border shadow-sm hover:shadow-md transition-shadow duration-200",
        className,
      )}
      {...props}
    />
...
function CardHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "grid auto-rows-min items-start gap-2 px-6 pt-6 pb-4",
        className,
      )}
      {...props}
    />
