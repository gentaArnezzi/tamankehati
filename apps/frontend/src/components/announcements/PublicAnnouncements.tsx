...
export function PublicAnnouncements({
  ...
  const [selectedTypeFilter, setSelectedTypeFilter] = useState<string>(typeFilter || 'all');
  ...
  const loadData = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/announcements/public?${params}`
      );
  ...
  useEffect(() => {
    loadData();
  }, [currentPage, searchQuery, selectedTypeFilter, showFeaturedOnly, showPinnedOnly, itemsPerPage]);
      ...
      <div className="grid gap-4">
        {announcements.map((announcement) => (
          <Card key={announcement.id} className="hover:shadow-md transition-shadow cursor-pointer">
            <CardContent className="p-6" onClick={() => handleView(announcement)}>
