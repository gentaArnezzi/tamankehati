...
export function ArtikelPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'https://tamankehati-backend-zxb9.onrender.com'}/api/v1/articles/?${params}`,
        {
          headers: {
  ...
  const handlePublish = async (artikel: Artikel) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'https://tamankehati-backend-zxb9.onrender.com'}/api/v1/articles/${artikel.id}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardContent className="pt-6">
            <div className="text-sm text-muted-foreground mb-1">Total Artikel</div>
            <div className="text-2xl">{data.length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="text-sm text-muted-foreground mb-1">Dipublikasikan</div>
            <div className="text-2xl">{data.filter(a => a.status === 'approved').length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="text-sm text-muted-foreground mb-1">Draft</div>
            <div className="text-2xl">{data.filter(a => a.status === 'draft').length}</div>
          </CardContent>
        </Card>
      </div>
          ...
          data.map((artikel) => (
            <Card key={artikel.id}>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground">{artikel.excerpt}</p>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => handleEdit(artikel)}>
                    <Pencil className="mr-2 h-4 w-4" />
                    Edit
                  </Button>
                  {artikel.status === 'draft' && (
                    <Button variant="outline" size="sm" onClick={() => handlePublish(artikel)}>
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {formMode === 'create' ? 'Buat artikel atau berita baru tentang keanekaragaman hayati.' : 'Perbarui konten artikel.'}
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
              <FormField control={form.control} name="judul" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="Masukkan judul artikel" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="excerpt" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Ringkasan singkat artikel (maksimal 200 karakter)" rows={2} {...field} />
                  </FormControl>
                  <FormDescription>Ringkasan ini akan muncul di halaman daftar artikel</FormDescription>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="konten" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Tulis konten artikel di sini (mendukung Markdown)" rows={12} {...field} />
                  </FormControl>
                  <FormDescription>Gunakan Markdown untuk format teks</FormDescription>
                  <FormMessage />
                </FormItem>
              )} />

              <FormField control={form.control} name="gambar_cover" render={({ field }) => (
                <FormItem>
              ...
              <DialogFooter>
                ...
                <Button type="submit" style={{ backgroundColor: '#233c2b' }}>
                  {formMode === 'create' ? 'Simpan Artikel' : 'Perbarui Artikel'}
                </Button>
              </DialogFooter>
            </form>
          </Form>
